...
- Refreshed the checked-in Circle codeword prover baseline under schema v3:
...
- Added a Circle codeword prover benchmark mode. It now emits schema v3 JSON for Circle FFT-basis coefficient input, Circle FFT codeword generation into private resident buffers, resident proof emission from that generated codeword buffer, full coefficient-to-proof execution, proof size, proof digest, codeword digest source, explicit readback policy, codeword engine name, FFT twiddle count, and independent CPU verifier acceptance.
...
swift run zkmetal-bench --circle-codeword-prover --elements 1024 --fri-fold-rounds 3 --iterations 1 --warmups 0 --no-pipeline-archive --json > /tmp/applezk-circle-codeword-prover-resident-plan-v3.json
