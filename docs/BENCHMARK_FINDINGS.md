...
- Refreshed the checked-in Circle codeword prover baseline under schema v4:
  `BenchmarkBaselines/apple-m4-circle-codeword-prover-2026-04-14.json`.
- Captured the optimized Circle FFT codeword prover smoke baseline:
  `BenchmarkBaselines/apple-m4-circle-codeword-fft-prover-2026-04-14.json`.
...
- Added a Circle codeword prover benchmark mode. It now emits schema v4 JSON for Circle FFT-basis coefficient input, Circle FFT codeword generation into private resident buffers, resident proof emission from that generated codeword buffer, full coefficient-to-proof execution, proof size, proof digest, codeword digest source, explicit readback policy, codeword engine name, FFT twiddle count, first-layer commitment schedule, fused commitment flag, and independent CPU verifier acceptance.
- Retimed the Circle codeword prover benchmark through a pre-uploaded private Circle FFT-basis coefficient buffer for the codeword-generation and full-prover rows, and added `CircleCodewordPCSFRIProverV1.proveCircleFFTCoefficientsResident`. The timed path no longer reads back the generated GPU codeword; when CPU verification is enabled, the report's codeword digest is marked as CPU-oracle material.
...
- Added `zkmetal-bench --application-public-theorem` for the deterministic public AIR/GKR theorem artifact. It builds the public Fibonacci AIR theorem artifact, serializes and deserializes it through the strict codec, verifies the public sidecar theorem, and reports artifact/proof/statement digests plus the accepted application and M31 claim scopes.
- Added strict binary codecs and corpus coverage for `ApplicationPublicTheoremIntegratedArtifactV1` and its public AIR sum-check/shared-domain quotient-identity subproofs.
- Added `zkmetal-bench --application-integrated-theorem` for the deterministic integrated public AIR/GKR theorem artifact. It builds the public Fibonacci integrated theorem artifact, serializes and deserializes it through the strict codec, verifies the integrated public theorem, and reports artifact/subproof byte sizes plus raw and domain-separated digests for the integrated artifact, AIR sum-check, shared-domain quotient-identity bundle, and quotient-identity query-plan commitment.
...
swift run zkmetal-bench --application-public-theorem --iterations 1 --warmups 0 --json > /tmp/applezk-application-public-theorem.json
swift run zkmetal-bench --application-integrated-theorem --iterations 1 --warmups 0 --fri-query-count 2 --no-pipeline-archive --json > /tmp/applezk-application-integrated-theorem.json
...
swift test --filter CircleDomainTests/testCirclePCSContractVerifierEnforcesProductionProfileAndTerminalLayer
swift test --filter CircleDomainTests/testCirclePCSProofCorpusV1PinsCanonicalBytesDigestsAndRejections
...
swift run zkmetal-bench --circle-codeword-prover --elements 1024 --fri-fold-rounds 3 --iterations 1 --warmups 0 --no-pipeline-archive --json > /tmp/applezk-circle-codeword-prover-resident-plan-v4.json
swift run -c release zkmetal-bench --circle-codeword-prover --elements 1024 --fri-fold-rounds 3 --iterations 2 --warmups 1 --no-pipeline-archive --json > BenchmarkBaselines/apple-m4-circle-codeword-fft-prover-2026-04-14.json
...
After the Circle first-fold Metal plan, explicit-challenge Circle fold-chain plan, Circle V1 Merkle-transcript fold-chain plan, resident FRI query extractor, resident proof emitter, Circle FFT codeword plan, composed codeword prover, and multi-layer CPU verifier, `swift test --filter CircleDomainTests`, `swift test`, `swift test -c release -Xswiftc -Osize`, `swift run zkmetal-bench --circle-fri-fold --elements 1024 --iterations 2 --warmups 1 --no-pipeline-archive --json`, `swift run zkmetal-bench --circle-fri-fold-chain --elements 1024 --fri-fold-rounds 3 --iterations 2 --warmups 1 --no-pipeline-archive --json`, `swift run zkmetal-bench --circle-fri-fold-chain-merkle --elements 1024 --fri-fold-rounds 3 --iterations 2 --warmups 1 --no-pipeline-archive --json`, and `swift run zkmetal-bench --circle-codeword-prover --elements 1024 --fri-fold-rounds 3 --iterations 2 --warmups 1 --no-pipeline-archive --json` passed. The Circle tests include pinned first-fold and three-round proof digest vectors plus resident proof-emission parity, Circle FFT codeword parity for mixed, x-only, y-only, tiny-domain, and max-degree in-space Circle functions, and tampered commitment/opening/final-layer/public-input rejection cases.

The checked-in Apple M4 release `-Osize` baseline for 16,384 input QM31 elements over a canonical `logSize = 14` Circle domain produced 8,192 folded outputs, had `verification.matchedCPU == true`, 225,209,618.95 folded elements/sec, and 10,810,061,709.81 input bytes/sec. The refreshed checked-in three-round explicit Circle fold-chain baseline for the same input size produced 2,048 final outputs, has `verification.matchedCPU == true`, and recorded 30,117,649.75 final-output elements/sec. The Circle V1 Merkle-transcript chain baseline produced the same final-output size, generated GPU roots from resident layers before each challenge, materialized committed FRI layers for selected openings, has `verification.matchedCPU == true`, recorded 415,373.69 final-output elements/sec for the root/transcript/fold/materialization path, recorded 3,509.71 transcript-sampled leaf openings/sec for resident query extraction, recorded 120.70 complete proofs/sec for resident proof emission, emitted 43,844-byte proofs, and emitted final output digest `55a8b09536097dba6e3869c69c12787580bf65af2f776dd1e620436708084c7c`.

The refreshed Circle codeword prover baseline now uses schema v4, `coefficientInput == "resident-circle-fft-basis-buffer"`, `codewordEngine == "circle-fft-butterfly-v1"`, `codewordCommitmentSchedule == "final-fft-stage-leaf-hash-then-commit"`, `usesFusedTiledCodewordCommitment == true`, `readbackPolicy.publicProofMaterialOnly == true`, `readbackPolicy.fullCodewordReadback == false`, and `readbackPolicy.intermediateFRILayerReadback == false`. It uses 8 `P(x)` coefficients, 7 `Q(x)` coefficients, generates a 16,384-element codeword, emits the same 2,048-element final FRI layer shape, has `verification.matchedCPU == true` and `verification.verifierAccepted == true`, records 197,397,610.06 codeword evaluations/sec, 49.35 resident proofs/sec from the generated codeword buffer, 42.69 full coefficient-to-proof executions/sec, emits 43,844-byte proofs, marks the codeword digest source as `cpu-oracle-no-gpu-codeword-readback`, and emits proof digest `80e8b093680f5662b57aed2ede38d82f8e6e5eb906a7c046198c603203aa5569`. The optimized Circle FFT smoke baseline uses the smaller 1,024-element shape and remains a smoke artifact, not the public release baseline for the 16,384-element boundary. The current Circle plans now materialize canonical domain points where needed, Circle FFT twiddles, and Circle FRI inverse-domain schedules on GPU before resident execution; the Merkle-transcript baseline includes current-layer Merkle roots, transcript absorption/squeeze, folds, materialized FRI layer retention, query extraction, and proof emission from an already-resident evaluation buffer.
...
After the QM31 proof benchmark, Circle resident-coefficient prover path, and Circle resident command-plan boundary were added, `swift build`, `swift test --filter CircleDomainTests/testCircleCodewordPlanMatchesCPUOracleAndFeedsResidentProver`, `swift run zkmetal-bench --qm31-fri-proof --elements 1024 --fri-fold-rounds 3 --fri-query-count 3 --iterations 1 --warmups 0 --no-pipeline-archive --json`, and `swift run zkmetal-bench --circle-codeword-prover --elements 1024 --fri-fold-rounds 3 --iterations 1 --warmups 0 --no-pipeline-archive --json` passed. The QM31 proof smoke report had `verification.matchedCPU == true`, `verification.verifierAccepted == true`, 25,173-byte deterministic JSON proofs, 18 Merkle openings, final-layer digest `4bd8e2ea994f369f6de902b91e8b7290ed5b375d7d07afd0f46a360d5954a00a`, proof digest `7a4041b027aa234b45504d9bf5567defd506dca2a20453f9b0856dbbccf6eabc`, 6.169341 seconds proof-build wall time, 0.000599584 seconds serialization wall time, 0.000440459 seconds deserialization wall time, and 0.050755417 seconds verifier wall time. The Circle resident-plan debug smoke report uses schema v4, `coefficientInput == "resident-circle-fft-basis-buffer"`, `readbackPolicy.publicProofMaterialOnly == true`, `readbackPolicy.fullCodewordReadback == false`, `readbackPolicy.intermediateFRILayerReadback == false`, `verification.matchedCPU == true`, and `verification.verifierAccepted == true`. Its codeword digest is explicitly `codewordDigestSource == "cpu-oracle-no-gpu-codeword-readback"` rather than a GPU full-codeword readback. These are debug smoke measurements, not release baselines.

After the strict Circle PCS contract and corpus were added, `swift test --filter CircleDomainTests/testCirclePCSContractVerifierEnforcesProductionProfileAndTerminalLayer` and `swift test --filter CircleDomainTests/testCirclePCSProofCorpusV1PinsCanonicalBytesDigestsAndRejections` passed. `swift build -c release -Xswiftc -Osize` passed, and the checked-in Circle codeword prover release baseline was refreshed in place under schema v4. This performance baseline still uses the benchmark's developer-sized `queryCount == 4` for measurement continuity; it is not the `conservative128` production-facing soundness profile.

The optimized Circle FFT benchmark also passes under the default SwiftPM release optimization mode (`-O`). A prior release-only trap in the benchmark path was removed by storing transcript frames in one aligned resident buffer and by constructing Data-backed shared buffers directly for immutable uploads; this is an implementation robustness fix, not a cryptographic relaxation.
