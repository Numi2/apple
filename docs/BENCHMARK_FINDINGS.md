...
- Added a verifier-facing linear QM31 FRI proof format with transcript-sampled query decommitments, deterministic serialization, and an independent CPU verifier. This is protocol correctness work rather than a new GPU throughput path, so it is covered by tests and cryptographic findings instead of a new benchmark row.
