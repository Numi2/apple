...
private struct QM31VectorParams {
    var count: UInt32
    var operation: UInt32
    var fieldModulus: UInt32
}

public struct QM31VectorArithmeticResult: Sendable {
...
public final class QM31VectorArithmeticPlan: @unchecked Sendable {
    private static let defaultUploadRingSlotCount = 3

    public let operation: QM31VectorOperation
    ...
    public init(
        ...
        let elementByteCount = 4 * MemoryLayout<UInt32>.stride
        ...
        self.pipeline = try context.pipeline(
            for: KernelSpec(kernel: "qm31_vector_arithmetic", family: .scalar, queueMode: .metal3)
        )
        self.uploadRingLHS = try SharedUploadRing(
            device: context.device,
            slotCapacity: inputByteCount,
            slotCount: Self.defaultUploadRingSlotCount,
            label: "AppleZKProver.QM31VectorUploadLHS"
        )
        self.uploadRingRHS = try SharedUploadRing(
            device: context.device,
            slotCapacity: inputByteCount,
            slotCount: Self.defaultUploadRingSlotCount,
            label: "AppleZKProver.QM31VectorUploadRHS"
        )
        self.arena = try ResidencyArena(
            ...
            label: "AppleZKProver.QM31VectorArena"
        )
        ...
        self.outputReadback = try MetalBufferFactory.makeSharedBuffer(
            ...
            label: "AppleZKProver.QM31VectorReadback"
        )
    }
    ...
    public func execute(
        lhs: [QM31Element],
        rhs: [QM31Element]? = nil
    ) throws -> QM31VectorArithmeticResult {
    ...
    public func executeVerified(
        ...
        let expected = try QM31Field.apply(operation, lhs: lhs, rhs: rhs)
        let measured = try execute(lhs: lhs, rhs: rhs)
        guard measured.values == expected else {
            throw AppleZKProverError.correctnessValidationFailed("QM31 vector arithmetic GPU result did not match the CPU oracle.")
        }
        return measured
    }
    ...
    private func executeLocked(
        ...
        encoder.label = "QM31.VectorArithmetic.Kernel"
        ...
        encoder.setBytes(&params, length: MemoryLayout<QM31VectorParams>.stride, index: 3)
        ...
        readbackBlit.label = "QM31.VectorArithmetic.Readback"
        readbackBlit.copy(
        ...
        return QM31VectorArithmeticResult(
            values: Self.readQM31Buffer(outputReadback, count: count),
            stats: GPUExecutionStats(cpuWallSeconds: wall, gpuSeconds: gpuDuration(commandBuffer))
        )
    }

    private func validateInputs(lhs: [QM31Element], rhs: [QM31Element]?) throws {
        guard lhs.count == count else {
            throw AppleZKProverError.invalidInputLayout
        }
        try QM31Field.validateCanonical(lhs)
        if operation == .inverse, lhs.contains(where: QM31Field.isZero) {
            throw AppleZKProverError.invalidInputLayout
        }
        if operation.requiresRightHandSide {
            ...
            try QM31Field.validateCanonical(rhs)
        } else if rhs != nil {
            throw AppleZKProverError.invalidInputLayout
        }
    }
